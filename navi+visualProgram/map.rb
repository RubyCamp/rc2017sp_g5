...
class Map
  attr_accessor :data
  # マップチップ毎の画像イメージ
  CELL_IMAGES = Image.loadToArray(File.join(File.dirname(__FILE__), "images", "map_chips105.png"), 5, 5)
  ...
  def calc_route(start, goal)
    ...
    res = g.get_route(start_id, goal_id)
    #p res
    return res
  end
 ...
 def item
    return @item if @item
    item_y = @map_data.index{|map_x| map_x.include?(ITEM) }
    item_x = @map_data[item_y].index(ITEM)
    @item = [item_x, item_y]
    return @item 
  end
